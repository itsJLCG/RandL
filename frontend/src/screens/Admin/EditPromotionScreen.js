...
import { updatePromotion } from '../../actions/promotionActions';
import { fetchProducts } from '../../actions/productActions';

const EditPromotionScreen = ({ navigation, route }) => {
  const { promotion: initialPromotion } = route.params;
  ...
  const [promotion, setPromotion] = useState({
    ...
    discountPercentage: initialPromotion.discountPercentage.toString(),
  ...
  useEffect(() => {
    dispatch(fetchProducts()).then(result => {
      if (result.success) {
        // Find and set previously selected products
        const initialSelected = result.products.filter(product => 
          initialPromotion.products.some(p => 
            typeof p === 'object' ? p._id === product._id : p === product._id
          )
        );
        setSelectedProducts(initialSelected);
      }
    });
  }, [dispatch, initialPromotion.products]);
  ...
  const handleSubmit = async () => {
    if (validateForm()) {
      ...
      try {
        ...
        const result = await dispatch(updatePromotion(initialPromotion._id, {
          ...promotion,
          products: productIds,
        ...
        if (result.success) {
          ...
          Alert.alert('Error', result.message || 'Failed to update promotion');
        }
      } catch (error) {
      ...
      <View style={styles.footer}>
        ...
        >
          {isLoading ? (
            ...
            <Text style={styles.saveButtonText}>Update Promotion</Text>
          )}
        </TouchableOpacity>
      </View>
